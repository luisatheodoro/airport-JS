describe("Plane", function () {
  ...
  describe("#land", function () {
    it("Plane cannot land if plane is already in land", function () {
      ...
      expect(function () {
        airFrance787.land(cityAirport)
      }).toThrowError("Cannot land, because plane is already landed");
    });
  });
  ...
  describe("#take_off", function () {
    it("Plane cannot take off if plane is already flying", function () {
      ...
      expect(function () {
        airFrance787.takeOff(cityAirport)
      }).toThrowError("Cannot take off, because plane has already taken off or is not in this airport");
    });

    it("Plane cannot take off from a different airport", function () {
      airFrance787.land(cityAirport);
      expect(function () {
        airFrance787.takeOff(gatwickAirport)
      }).toThrowError("Cannot take off, because plane has already taken off or is not in this airport");
    });
  });
